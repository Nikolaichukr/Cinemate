...
def movies_page():
    ...
    if filter_form.validate_on_submit():
        ...
        if not movie_list:
            empty_message = f"There are no movies between the {from_year} and the {to_year} in our database."
    return render_template('movies.html', title="List of Movies", movie_list=movie_list, filter_form=filter_form,
...
def add_movie():
    ...
    if form.validate_on_submit():
        movie_service.add_movie(title=form.title.data, year=form.year.data, director=form.director.data,
...
def update_movie(movie_id):
    ...
    if current_movie is not None:
        ...
        if form.validate_on_submit():
            movie_service.update_movie(current_movie=current_movie, title=form.title.data, year=form.year.data,
        ...
        return redirect(url_for("movies.movies_page"))
    return render_template('update_movie.html', title="Update Movie Info", form=form, movie=current_movie)
